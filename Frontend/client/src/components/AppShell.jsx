...
function AppShell({ user, activePage, onNavigate, onLogout, children }) {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7f6] text-[#17252a]">
      {mobileOpen && (
        <button
          className="fixed inset-0 z-30 bg-[#17252a]/35 lg:hidden"
          type="button"
      ...
      >
        <div className="flex h-[72px] items-center justify-between border-b border-white/10 px-5">
          ...
          <button className="text-[#a9ccc5] lg:hidden" type="button" onClick={() => setMobileOpen(false)}>
            <X size={18} />
          </button>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-6">
          {!collapsed && (
            <p className="px-3 pb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-[#8bbdb5]">
              Workspace
            </p>
          )}
          {visibleNavigation.map(({ id, label, icon: Icon }) => (
            ...
            >
              ...
              {!collapsed && activePage === id && (
                <ChevronRight size={15} className="ml-auto text-[#f2c14e]" />
              )}
            </button>
          ))}
        </nav>
      ...
      <div className={`min-h-screen transition-all duration-200 ${collapsed ? 'lg:pl-[76px]' : 'lg:pl-[252px]'}`}>
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-[#dce5e2] bg-white/90 px-5 backdrop-blur sm:px-8">
          <div className="flex items-center gap-3">
            <button className="text-[#718087] lg:hidden" type="button" onClick={() => setMobileOpen(true)}>
              <Menu size={21} />
            </button>
            <button
              className="hidden text-[#718087] lg:block"
              type="button"
            ...
            <div className="flex items-center gap-2 text-sm text-[#718087]">
              ...
              <strong className="text-[#17252a]">
                {navigation.find((item) => item.id === activePage)?.label || 'Account settings'}
              </strong>
            </div>
          </div>
          <div className="flex items-center gap-3">
